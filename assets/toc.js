(function () {
  ...
  function tocLink(node) {
    while (node) {
      if (
        node.nodeType === 1 &&
        node.classList &&
        (node.classList.contains("okmate-toc-link") ||
          node.classList.contains("okmate-outline-link"))
      ) {
        return node;
      }
  ...
  function tocLinks() {
    return document.querySelectorAll(
      ".okmate-toc-link[href^='#'], .okmate-outline-link[href^='#']"
    );
  }
  ...
  function observeMain() {
    ...
    if (!main || main.__okmateTocObserved) {
      return;
    }
    main.__okmateTocObserved = true;
  ...
  window.__okmateToc = { enhance: enhance };
})();
