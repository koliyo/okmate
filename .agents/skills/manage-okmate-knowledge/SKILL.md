---
name: manage-okmate-knowledge
description: Query, inspect, validate, author, or review Okmate's `knowledge/` Open Knowledge Format bundle with `okmate`. Use for architecture and decision retrieval, knowledge-record edits, lifecycle or provenance review, graph inspection, and OKF diagnostics. Do not use for ordinary source-code changes unless they also require consulting or updating canonical knowledge.
---

# Manage Okmate Knowledge

Use this repository's checked-in `knowledge/` bundle as the canonical database
and the `okmate` binary as its deterministic interface (`okmate` on `PATH`, or
`cargo run -q --no-default-features -p okmate --` from the repo root). Keep
domain facts in the bundle rather than copying them into this skill. Engine
tests live here (`cargo test -p okf` / `cargo test -p okmate --no-default-features`).
...
1. Choose a type collection and area based on the claim's purpose and
   ...
   `<area>` is one of `okf`, `okmate`, or `ops`. Pick the primary owner of the
   work, not the union of tags. Architecture, decisions, status, reference,
   design, and case-studies stay flat. Deepen in place later; do not flatten
   or nest by lifecycle.
   ...
   tags: [domain/okmate, concern/architecture]
   status: draft
   generated: { by: process:cursor, at: 2026-08-26T00:00:00Z }
...
2. Keep record bodies inert Markdown. Do not add Rocdown declarations, raw
   HTML, wikilinks, or executable content.
3. Preserve unknown OKF metadata unless the task explicitly removes it.
4. Give each source a unique `id`, use paths relative to the record, and attach
   a matching keyed footnote to every sourced claim. Keep source IDs and
   footnote IDs synchronized.
5. On a substantive generated revision, update `generated.at`, set the record
   to `draft`, and retain historical verification events. Never invent or
   advance a human verification event.
6. Update the nearest directory `index.md` when adding, moving, or removing a
...
Run the strict profile after every knowledge edit:
...
If parser, validation, inspection, or CLI behavior changed:
