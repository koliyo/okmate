...
4. On conflict, inspect each file and both sides. If the correct resolution
   ...
   theirs` as a blanket strategy. If `knowledge/log.md` uses Git
   `merge=union`, concurrent **new** bullets should auto-combine; leftover
   conflicts or duplicated rewrites still need a human look. Collection
   `index.md` files are ordinary merges.
